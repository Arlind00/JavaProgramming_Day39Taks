 ...
 */
        ...
        Map<Integer, ArrayList<String>> groups = new TreeMap<>();       // pair: Id & names of students
        groups.put(1, group1);          //   (key (group no.), group elements(values))
        groups.put(2, group2 );
        groups.put(9, group9);
        ...
        */
        ...
        System.out.println(groups.get(2).get(1).charAt(1));           // for getting second student from group 2


        System.out.println("------------------------------------------------");

        for (ArrayList<String> eachGroup : groups.values()) {           // each group is run
            for (String eachName : eachGroup) {                         // each element of each group is run
